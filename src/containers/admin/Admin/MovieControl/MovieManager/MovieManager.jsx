...
export default class MovieManager extends Component {
  ...
  handleGetListSearchMovie(page) {
    movieApi
      .fetchSearchMovieInPageApi(this.state.keySearch, page)
      .then((res) =>
    ...
    movieApi
      ...
      .then((res) => {
        ...
        if (this.state.listMovieSearchInPage) {
          this.handleGetListSearchMovie(
            this.state.listMovieSearchInPage.currentPage
          );
        } else {
  ...
  handleSearchMovieInPage(e) {
    ...
    if (!e.target[0].value) {
      ...
      movieApi
        .fetchSearchMovieInPageApi(e.target[0].value, startPage)
        .then((res) =>
          this.setState({
        ...
        <div className="mr-4 mb-0">
          <ul className="nav nav-pills mb-3" id="pills-tab" role="tablist">
            ...
            <li className="nav-item" role="presentation">
              <Link
                ...
                className="nav-link btn btn btn-light ml-2"
                role="tab"
              >
                Thêm phim
              </Link>
          ...
          <div className="movie__manage">
            <form
              ...
              onSubmit={(e) => this.handleSearchMovieInPage(e)}
            >
              <input
                type="text"
                      ...
                      <tr key={movie.maPhim}>
                        ...
                        <td>
                          <div style={{ display: "inline" }} className="mr-2">
                            <Link
                              to={{
                                pathname: `/admin/movies/movie-showtime/${movie.maPhim}`,
                                movieInformation: movie,
                              }}
                            >
                              {" "}
                              <button className=" showTime__movie btn btn-primary">
                                <CalendarOutlined />
                              </button>{" "}
                            </Link>
                            &thinsp;
                            <Link
                              to={`/admin/movies/edit-movie/${movie.maPhim}`}
                            >
                              {" "}
                              <button className="edit__movie btn btn-success">
                                <EditOutlined />
                              </button>{" "}
                            </Link>
                            &thinsp;
                            <button
                              className="delete__movie btn btn-danger"
                              onClick={() =>
                                this.handleDeleteMovie(movie.maPhim)
                              }
                            >
                              <CloseOutlined />
                            </button>
            ...
            <div className="navigate__user text-right">
              <ul className="d-block">
                <button
                  ...
                  disabled={listMovieRender.currentPage === 1}
                  onClick={() =>
                    this.handlePrevPagination(listMovieRender.currentPage)
                  }
                >
                  Previous
                </button>
                {this.handlePagination(listMovieRender.totalPages).map(
                  (page) => {
                    return (
                      <li
                        className={`btn btn-outline-dark mr-1 ${
                          listMovieRender.currentPage === page && "active"
                        }`}
                        key={page}
                        onClick={() => {
                ...
                <button
                  ...
                  disabled={
                    listMovieRender.currentPage === listMovieRender.totalPages
                  }
                  onClick={() =>
                    this.handleNextPagination(listMovieRender.currentPage)
                  }
                >
                  Next
                </button>
